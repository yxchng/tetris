...
public class State {
	...
	public State(int nextPiece, int turn, int field[][], int top[]) {
        copyField(field);
        this.top = top.clone();
	...
	public double getVariation() {
        ...
        for (int i = 0; i< 10; i++){
            ...
            sum += getColumnHeight(i);
            if (getColumnHeight(i) > maximumHeight){
                maximumHeight = getColumnHeight(i);
            }
            if (getColumnHeight(i) < minimumHeight){
                minimumHeight = getColumnHeight(i);
            }
        }
        mean = sum / 10;
        variation = 0;
        for (int i = 0; i< 10; i++) {
            variation += (mean - getColumnHeight(i)) * (mean - getColumnHeight(i));
        }
        return variation;
	}
	...
	public double getColStdDev() {
		...
		for (int j = 0; j < 10; j++) {
			colStdDev += Math.pow((top[j] - totalHeight / 10), 2);
		}

		colStdDev = Math.sqrt(colStdDev / 10);
